...
Putting it all together, we can find our final probability $P(N | M) = \frac{0.3^4}{4!}e^{-0.3} \div (1 - e^{-0.3})$ = \framebox[1.1\width]{\textbf{0.00096467487}}
...
\subsection*{(c)}
By definition of geometric distribution, we know that for some geometric distribution $Y \sim $ G($p$), where $Y$ is the round at which we get our first success, E($Y$) = $\frac{1}{p}$, and since our $p = \frac{1}{2}$, E$(Y) = \frac{1}{\frac{1}{2}} = 2$. However, in this context, this means that we will successfully win at the 2nd round, which is not the last round where we lose money, so our net winnings $X$ is going to be \framebox[1.1\width]{\textbf{1 dollar}}. We can verify this by seeing that at round 2, we win 2 dollars, but have lost 1 dollar previously for a total of 2 - 1 = 1 dollar.
